...
export function WebsiteHeader(){
 ...
 return <header className="website-header" lang={english?'en':'hi'}><div className="website-nav">
  ...
  <div className="website-nav-actions"><div className="website-language" role="group" aria-label="Website language"><button disabled={!ready} onClick={()=>setLanguage('english')} aria-pressed={english} type="button" lang="en" aria-label="English">EN</button><button disabled={!ready} onClick={()=>setLanguage('hinglish')} aria-pressed={!english} type="button" lang="hi" aria-label="हिन्दी">हिं</button></div><Link className="website-login rounded-xl border border-blue-600 bg-blue-600 px-3.5 py-2.5 !text-white shadow-sm transition hover:border-blue-700 hover:bg-blue-700" href="/app" aria-current={pathname?.startsWith('/app')?'page':undefined}>{accountLabel}<ArrowUpRight size={15}/></Link></div>
