...
import { ArrowRight, BookOpen, Cable, Calculator, FileSpreadsheet, Files } from "lucide-react";
...
export const metadata: Metadata = {
  ...
  description: "Choose Meesho, Amazon India, Flipkart, Shopify or WooCommerce and open its SellerHisab file, connection, guide and calculator tools from one marketplace hub.",
  alternates: { canonical: "/marketplaces" },
};

const STATE_STYLES: Record<CapabilityState | "file-first", string> = {
  ...
  "file-first": "border-emerald-200 bg-emerald-50 text-emerald-800",
};

function statusText(state: CapabilityState | "file-first") {
  if (state === "live" || state === "file-first") return "Live";
  if (state === "available") return "Available";
  if (state === "activation-required") return "Auth Required";
  return "Not Claimed";
}

function CompactRow({ icon, label, state }: { icon: ReactNode; label: string; state: CapabilityState | "file-first" }) {
  return (
...
function ConnectionRow({ marketplace }: { marketplace: MarketplaceDefinition }) {
  if (marketplace.connection.state === "not-available") {
    return <CompactRow icon={<Files className="size-4 text-blue-600" />} label="File First Only" state="file-first" />;
  }
  return <CompactRow icon={<Cable className="size-4 text-blue-600" />} label="API Connect" state={marketplace.connection.state} />;
}
      ...
      <main className="website-editorial px-4 py-10 sm:px-6 md:py-14 lg:px-10">
        <div className="mx-auto max-w-[1280px]">
          <section className="mx-auto max-w-4xl text-center">
            ...
            <p className="mx-auto mt-4 max-w-3xl text-base leading-7 text-slate-600 sm:text-lg">Choose where you sell, then use that marketplace's analysis, connection, guides and calculators from one place.</p>
          </section>
              ...
              <article key={marketplace.id} className="flex h-full flex-col rounded-[24px] border border-slate-200/80 bg-white/85 p-5 shadow-sm">
                <div className="flex items-center justify-between gap-4 border-b border-slate-100 pb-4">
                  <div className="flex min-w-0 items-center gap-4">
                    <div className="flex h-14 w-32 shrink-0 items-center"><Image src={marketplace.logo} alt={`${marketplace.name} logo`} width={145} height={56} className="max-h-12 w-auto max-w-[128px] object-contain object-left" /></div>
                ...
                <div className="mt-2">
                  ...
                  <ConnectionRow marketplace={marketplace} />
