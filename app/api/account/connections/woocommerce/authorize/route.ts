...
export async function POST(request:Request){
 ...
 try{
  await requireWorkspaceCapability(user,'connector_manage');await enforceRateLimit(request,'wc-auth-start',user.id,8,3600);
  ...
  const url=new URL(storeUrl+'/wc-auth/v1/authorize');url.search=new URLSearchParams({app_name:'SellerHisab',scope:'read',user_id:state,return_url:origin+'/app/connections',callback_url:origin+'/api/connectors/woocommerce/callback'}).toString();
  return Response.json({authorizationUrl:url.toString(),syncRequiresPlan:'pro'},{headers:{'cache-control':'no-store'}});
 }catch{return Response.json({error:'Read-only authorization could not be started. Check your store and workspace permissions.'},{status:400});}
}
