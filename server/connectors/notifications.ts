...
export async function dispatchConnectorNotifications(){
 ...
 // Oldest job creation is also a pacing clock for failed connections.
 ...
 for(const row of stale.results??[])await enqueueConnectorSyncJob({connectionId:row.id,tenantId:row.tenantId,requestedByUserId:row.userId,days:7,orderDateField:row.connectorId==='shopify-v1'?'updated_at':undefined});
 ...
 for(const row of rows.results??[]){const job=await enqueueConnectorSyncJob({tenantId:row.tenantId,connectionId:row.connectionId,requestedByUserId:row.userId,days:row.days,orderDateField:'updated_at'});
